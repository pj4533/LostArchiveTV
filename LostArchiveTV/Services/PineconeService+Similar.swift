...
extension PineconeService {
    ...
    func findSimilarByIdentifier(_ identifier: String, topK: Int = 20) async throws -> [SearchResult] {
        // Log that we're finding similar videos
        Logger.network.debug("Finding similar videos for identifier: \(identifier)")
        ...
        return try await query(vector: vector, topK: topK)
    }
